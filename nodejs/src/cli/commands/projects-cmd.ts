...
export interface ProjectsCmdOptions {
   ...
   * - `all` (default): remote list unchanged (still enriched)
...
async function loadDeployedByProject(
  ...
  try {
    ...
    // Rows with a deployed status elsewhere may arrive after a non-deployed row —
    // second pass prefers deployed.
    for (const d of listed.items) {
...
export async function runProjectsList(options: ProjectsCmdOptions = {}): Promise<void> {
  ...
  try {
    ...
    const known_local_paths = new Map(knownPresent.map(w => [w.project_id, w.path]));
...
export async function runProjectsGet(
  ...
  try {
    ...
    // Keep get readable: project body + status enrichment (+ latest deployment id when known).

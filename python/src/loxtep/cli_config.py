...
def load_config() -> dict[str, Optional[str]]:
    """
    Load config from env and ~/.loxtep/config.json. Same precedence as Node: env > file.
...
def load_credentials() -> Optional[dict[str, str]]:
    """
    Load credentials from ~/.loxtep/credentials.json. Returns dict with access_token (and optionally
    refresh_token, expires_at) or None if missing/invalid.
    """
...
def get_token_from_env_or_file() -> Optional[str]:
    """Token from LOXTEP_TOKEN env or ~/.loxtep/credentials.json access_token."""
